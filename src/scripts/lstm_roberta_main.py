# src/scripts/lstm_roberta_main.py (Modified for new data loading and splitting)
...
import numpy as np
import pandas as pd # Import pandas for concatenating
    ...
    try:
        ...
        if 'lstm_roberta' in config:
            ...
            for key in float_keys_model:
                if key in lr_cfg:
                    try: lr_cfg[key] = float(lr_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'lstm_roberta.{key}' to float. Value: {lr_cfg[key]}")
            for key in int_keys_model:
                 if key in lr_cfg:
                    try: lr_cfg[key] = int(lr_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'lstm_roberta.{key}' to int. Value: {lr_cfg[key]}")
            for key in bool_keys_model:
                 if key in lr_cfg:
                    try:
                        if isinstance(lr_cfg[key], str):
                            val_lower = lr_cfg[key].lower()
                            if val_lower in ['true', 'yes', 'on', '1']: lr_cfg[key] = True
                            elif val_lower in ['false', 'no', 'off', '0']: lr_cfg[key] = False
                            else: raise ValueError("Invalid boolean string")
                        else: lr_cfg[key] = bool(lr_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'lstm_roberta.{key}' to bool. Value: {lr_cfg[key]}")
            if 'hpo' in lr_cfg:
                hpo_cfg = lr_cfg['hpo']
                ...
                for key in int_keys_hpo:
                    if key in hpo_cfg:
                        ...
                        except (ValueError, TypeError): logger.warning(f"Could not convert config key 'lstm_roberta.hpo.{key}' to int. Value: {hpo_cfg[key]}")
...
def objective(trial, train_texts_hpo, train_labels_hpo, val_texts_hpo, val_labels_hpo, model_config, hpo_config):
    ...
    try:
        ...
        model = HybridSentimentClassifier(
            ...
            freeze_roberta=freeze_roberta, device=DEVICE, num_attention_heads=num_attention_heads,
            use_layer_norm=use_layer_norm, use_residual=use_residual, use_pooler_output=use_pooler_output,
...
def main():
    ...
    # Apply CLI overrides
    # Use specific sample sizes for train/test if provided
    sample_train = args.sample_size_train if args.sample_size_train is not None else model_config.get('base_sample_size', None) # Default to None to load all
    sample_test = args.sample_size_test if args.sample_size_test is not None else None # Default to None to load all
    ...
    model_save_path = os.path.join(models_dir, 'lstm_roberta_model_final.pt') # Keep original name for now

    # --- Load Data using the updated DataProcessor ---
    logger.info(f"Loading train/test data...")
    data_processor = DataProcessor() # Uses default DATA_DIR
    try:
        # Load train.csv and test.csv separately
    ...
    # --- Prepare Data Splits ---
    # Extract texts and labels from the loaded DataFrames
    # The 'label' column is created by load_and_preprocess_csv in DataManager
    train_texts_all = train_df['text_clean'].tolist()
    train_labels_all = train_df['label'].tolist()
    test_texts = test_df['text_clean'].tolist()
    test_labels = test_df['label'].tolist() # Assuming test set has labels for evaluation

    # Split the training data (from train.csv) into training and validation sets
    try:
        val_size = float(global_config.get('hpo_validation_size', 0.25)) # Use hpo_validation_size for train/val split
    ...
    # --- HPO Setup (Uses the new train/val splits) ---
    ...
    if args.mode == 'optimize' and run_hpo:
        ...
        study = optuna.create_study(
            study_name=f"lstm-roberta-hpo-{time.strftime('%Y%m%d-%H%M')}", direction="maximize",
            storage=storage_name, load_if_exists=True)
        logger.info(f"Using Optuna storage: {storage_name}")
        try:
            # --- Pass the correct train/validation splits to the objective function ---
            study.optimize(
    ...
    if args.mode == 'optimize' or args.mode == 'train':
        ...
        try:
            final_model = HybridSentimentClassifier(
                ...
                num_attention_heads=final_params.get('num_attention_heads', 4), use_layer_norm=final_params.get('use_layer_norm', True),
                use_residual=final_params.get('use_residual', True), use_pooler_output=final_params.get('use_pooler_output', False),
    ...
    if model_to_evaluate:
        logger.info("Evaluating final model on the reserved test set...")
        try:
            # Use the test_texts and test_labels loaded earlier
            test_predictions_raw = model_to_evaluate.predict(test_texts)
            # Map predictions (adjust if your model outputs different labels)
            # Assuming 'Positive' maps to 2, 'Negative' to 0, 'Neutral' to 1 based on data_processor
            pred_map = {'Negative': 0, 'Neutral': 1, 'Positive': 2}
            test_pred_labels = [pred_map.get(pred['label'], -1) for pred in test_predictions_raw] # Use -1 for unknown

            # Filter out invalid labels if necessary (e.g., -1 from mapping or test set)
            ...
            if not test_labels_filtered:
                ...
                try:
                    cm = sk_confusion_matrix(test_labels_filtered, test_pred_labels_filtered, labels=[0, 1, 2]) # Specify labels
                ...
                save_model_results(
                    model_name="LSTMRoBERTa_Final", metrics=metrics, parameters=params_to_save, example_predictions={})
