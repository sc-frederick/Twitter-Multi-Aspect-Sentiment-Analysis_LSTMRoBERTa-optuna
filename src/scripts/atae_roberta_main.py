# src/scripts/atae_roberta_main.py (Adapted for new data loading and splitting)
...
def objective(trial, train_texts_hpo, train_labels_hpo, val_texts_hpo, val_labels_hpo, model_config, hpo_config):
    ...
    try:
        ...
        model = ATAERoBERTaSentimentClassifier(
            ...
            lstm_hidden_dim=lstm_hidden_dim, lstm_layers=lstm_layers, dropout=dropout, learning_rate=lr,
            batch_size=batch_size, max_length=max_length, num_epochs=int(hpo_config['hpo_epochs']),
            freeze_roberta=freeze_roberta, device=DEVICE, bidirectional_lstm=bidirectional_lstm,
            use_layer_norm=use_layer_norm, weight_decay=weight_decay, use_scheduler=use_scheduler,
            scheduler_warmup_steps=scheduler_warmup_steps, gradient_accumulation_steps=gradient_accumulation_steps,
...
def main():
    ...
    int_keys_final = ['lstm_hidden_dim', 'lstm_layers', 'batch_size', 'max_length', 'scheduler_warmup_steps', 'gradient_accumulation_steps']
    bool_keys_final = ['freeze_roberta', 'bidirectional_lstm', 'use_layer_norm', 'use_scheduler']
    ...
    if args.mode == 'optimize' or args.mode == 'train':
        ...
        try:
            final_model = ATAERoBERTaSentimentClassifier(
                ...
                lstm_hidden_dim=final_params['lstm_hidden_dim'], lstm_layers=final_params['lstm_layers'], dropout=final_params['dropout'],
                ...
                bidirectional_lstm=final_params.get('bidirectional_lstm', True), use_layer_norm=final_params.get('use_layer_norm', True),
        ...
        if os.path.exists(model_save_path):
             model_config_for_load = CONFIG.get('atae_roberta', {}) # Load relevant config section
             try:
                 # Instantiate with ARCHITECTURE params before loading weights
                 model_to_evaluate = ATAERoBERTaSentimentClassifier(
                     roberta_model=model_config_for_load.get('roberta_model', 'roberta-base'),
                     lstm_hidden_dim=int(model_config_for_load.get('lstm_hidden_dim', 256)),
                     lstm_layers=int(model_config_for_load.get('lstm_layers', 1)),
                     dropout=float(model_config_for_load.get('dropout', 0.3)),
                     bidirectional_lstm=bool(model_config_for_load.get('bidirectional_lstm', True)),
    ...
    if model_to_evaluate:
        logger.info("Evaluating final ATAE-RoBERTa model on the reserved test set...")
        try:
        ...
        logger.warning("No ATAE-RoBERTa model was available for final evaluation.")
