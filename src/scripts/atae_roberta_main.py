# src/scripts/atae_roberta_main.py (Adapted for ATAE-RoBERTa)
...
try:
    ...
    # Import the new ATAE-RoBERTa classifier and compatible dataset
    from utils.atae_roberta_classifier import ATAERoBERTaSentimentClassifier, SentimentDataset
    from utils.results_tracker import save_model_results
except ImportError as e:
    print(f"Error importing utility modules: {e}")
    print(f"Please ensure 'utils' directory contains 'atae_roberta_classifier.py' and is importable.")
    sys.exit(1)
...
def load_config():
    ...
    try:
        ...
        # --- Type Casting for ATAE-RoBERTa Section ---
        if 'atae_roberta' in config:
            atae_cfg = config['atae_roberta']
            float_keys_model = ['learning_rate', 'dropout', 'weight_decay', 'max_grad_norm']
            int_keys_model = ['lstm_hidden_dim', 'lstm_layers', 'batch_size',
                             'max_length', 'scheduler_warmup_steps',
                             'gradient_accumulation_steps', 'final_epochs']
            bool_keys_model = ['freeze_roberta', 'bidirectional_lstm',
                              'use_layer_norm', 'use_scheduler']

            for key in float_keys_model:
                if key in atae_cfg:
                    try: atae_cfg[key] = float(atae_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'atae_roberta.{key}' to float. Value: {atae_cfg[key]}")
            for key in int_keys_model:
                 if key in atae_cfg:
                    try: atae_cfg[key] = int(atae_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'atae_roberta.{key}' to int. Value: {atae_cfg[key]}")
            for key in bool_keys_model:
                 if key in atae_cfg:
                    try:
                        if isinstance(atae_cfg[key], str):
                            val_lower = atae_cfg[key].lower()
                            if val_lower in ['true', 'yes', 'on', '1']: atae_cfg[key] = True
                            elif val_lower in ['false', 'no', 'off', '0']: atae_cfg[key] = False
                            else: raise ValueError("Invalid boolean string")
                        else: atae_cfg[key] = bool(atae_cfg[key])
                    except (ValueError, TypeError): logger.warning(f"Could not convert config key 'atae_roberta.{key}' to bool. Value: {atae_cfg[key]}")

            # Cast HPO settings
            if 'hpo' in atae_cfg:
                hpo_cfg = atae_cfg['hpo']
                int_keys_hpo = ['n_trials', 'timeout_per_trial', 'hpo_sample_size', 'hpo_epochs']
                for key in int_keys_hpo:
                    if key in hpo_cfg:
                        try: hpo_cfg[key] = int(hpo_cfg[key])
                        except (ValueError, TypeError): logger.warning(f"Could not convert config key 'atae_roberta.hpo.{key}' to int. Value: {hpo_cfg[key]}")
...
# --- Objective Function for Optuna (Adapted for ATAE-RoBERTa) ---
def objective(trial, train_texts, train_labels, val_texts, val_labels, model_config, hpo_config):
    """Optuna objective function to train and evaluate one ATAE-RoBERTa trial."""
    params_to_tune = {}
    ...
    # Combine suggested params with fixed params
    ...
    try:
        ...
        lstm_hidden_dim = int(current_params.get('lstm_hidden_dim', model_config.get('lstm_hidden_dim', 256)))
        lstm_layers = int(current_params.get('lstm_layers', model_config.get('lstm_layers', 1)))
        ...
        bidirectional_lstm = bool(current_params.get('bidirectional_lstm', model_config.get('bidirectional_lstm', True)))
        ...
        # --- Instantiate ATAE-RoBERTa Model for Trial ---
        model = ATAERoBERTaSentimentClassifier(
...
def main():
    parser = argparse.ArgumentParser(description="Optimize/Train/Test ATAE-RoBERTa model")
    ...
    # --- Load ATAE-RoBERTa specific config ---
    model_config = CONFIG.get('atae_roberta', {}) # <--- Changed section name
    if not model_config:
        logger.error("Configuration section 'atae_roberta' not found in config.yaml!")
        sys.exit(1)
    ...
    # --- Specific model path for ATAE-RoBERTa ---
    model_save_path = os.path.join(models_dir, 'atae_roberta_model_final.pt') # <--- Changed name
    ...
    # --- Optuna Optimization (ATAE-RoBERTa) ---
    best_params_from_hpo = {}
    study = None
    if args.mode == 'optimize' and run_hpo:
        logger.info(f"Starting ATAE-RoBERTa hyperparameter optimization with Optuna for {n_trials} trials...")
        # --- Use a different DB name for ATAE-RoBERTa study ---
        storage_name = f"sqlite:///{os.path.join(results_dir, 'optuna_atae_roberta.db')}" # <--- Changed DB name
        study = optuna.create_study(
            study_name=f"atae-roberta-hpo-{time.strftime('%Y%m%d-%H%M')}", direction="maximize",
    ...
    if args.mode == 'train' or args.mode == 'optimize':
        # Adapt keys based on ATAE-RoBERTa config
        float_keys_final = ['learning_rate', 'dropout', 'weight_decay', 'max_grad_norm']
        int_keys_final = ['lstm_hidden_dim', 'lstm_layers', 'batch_size',
                         'max_length', 'scheduler_warmup_steps', 'gradient_accumulation_steps']
        bool_keys_final = ['freeze_roberta', 'bidirectional_lstm',
                          'use_layer_norm', 'use_scheduler']
        try:
    ...
    if args.mode == 'optimize' or args.mode == 'train':
        logger.info("Training final ATAE-RoBERTa model...")
        # Log relevant params
        log_params = {k: final_params.get(k) for k in ['learning_rate', 'lstm_hidden_dim', 'lstm_layers', 'dropout', 'weight_decay', 'freeze_roberta', 'bidirectional_lstm']}
        logger.info(f"Using parameters: {log_params}")
        try:
            # --- Instantiate Final ATAE-RoBERTa Model ---
            final_model = ATAERoBERTaSentimentClassifier(
                roberta_model=str(final_params['roberta_model']),
                lstm_hidden_dim=final_params['lstm_hidden_dim'],
                ...
                device=DEVICE,
                bidirectional_lstm=final_params.get('bidirectional_lstm', True),
            ...
            # Save the final ATAE-RoBERTa model
            final_model.save_model(model_save_path)
            logger.info(f"Final ATAE-RoBERTa model saved to {model_save_path}")
            model_to_evaluate = final_model
        except Exception as train_err:
             logger.error(f"Failed during final ATAE-RoBERTa training: {train_err}", exc_info=True)
             model_to_evaluate = None

    # --- Load Model for Testing (Mode: test) ---
    elif args.mode == 'test':
        logger.info(f"Loading ATAE-RoBERTa model for testing from {model_save_path}...")
        if os.path.exists(model_save_path):
             # Instantiate a default wrapper; load_model handles architecture
             model_to_evaluate = ATAERoBERTaSentimentClassifier(device=DEVICE)
             try:
                 model_to_evaluate.load_model(model_save_path)
                 logger.info(f"ATAE-RoBERTa model loaded successfully from {model_save_path}")
             except Exception as e:
                 logger.error(f"Failed to load ATAE-RoBERTa model state: {e}", exc_info=True)
                 model_to_evaluate = None
        else:
            logger.error(f"No ATAE-RoBERTa model found at {model_save_path}. Cannot test.")
            model_to_evaluate = None
    ...
    if model_to_evaluate:
        if 'test_texts' not in locals() or 'test_labels' not in locals():
            ...
            logger.info("Evaluating ATAE-RoBERTa model on the test set...")
            try:
                ...
                logger.info("\nFinal Test Set Evaluation Results (ATAE-RoBERTa):")
                ...
                try:
                    ...
                    cm_path = os.path.join(results_dir, f'atae_roberta_{args.mode}_confusion_matrix.png') # Changed name
                ...
                # Save results
                # Extract config from loaded model if testing
                model_config_to_save = final_params if args.mode in ['train', 'optimize'] else model_to_evaluate.model.config if hasattr(model_to_evaluate, 'model') and hasattr(model_to_evaluate.model, 'config') else model_config
                ...
                params_to_save = {
                    ...
                    **model_config_to_save # Use loaded config if testing
                }
                params_to_save.pop('hpo', None) # Remove HPO sub-dict
                ...
                save_model_results(
                    model_name=f"ATAERoBERTa_{args.mode.capitalize()}", # Changed name
         ...
         logger.warning("No ATAE-RoBERTa model was available or trained/loaded for evaluation.")
